...
public class 힙정렬 {
	...
	public static void main(String[] args) {
		...
		int nx = x.length;
		heapSort(x, nx);
		System.out.println("오름차순으로 힙정렬하였습니다.");
		for (int aa : x) {
	...
	private static void downHeap(int[] a, int idx1, int idx2) {
		int t = a[idx1];
		a[idx1] = a[idx2];
		a[idx2] = t;
	}

	private static void swap(int[] a, int left, int right) { //left 3 right 6
